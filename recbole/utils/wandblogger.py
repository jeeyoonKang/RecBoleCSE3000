...
class WandbLogger(object):
    ...
    def log_group_eval(self, group_eval, group_name_mappings=None, group_eval_meta=None, step=None):
        """
        ...
        for group_type, group_metrics in group_eval.items():
            name_map = group_name_mappings.get(group_type, {}) if group_name_mappings else {}
            size_map = (
                group_eval_meta.get("user_group_sizes", {}).get(group_type + "_group", {}) or
                group_eval_meta.get("user_group_sizes", {}).get(group_type, {}) or
                group_eval_meta.get("item_group_sizes", {}).get(group_type + "_group", {})
                if group_eval_meta else {}
            )
        ...
        data = [[row["GroupType"], row["Group"], row["GroupSize"]] + [row.get(m) for m in all_metrics] for row in
                all_rows]
    ...
    def log_group_summary_table(self, group_type_metrics, step=None):
        ...
        table = self._wandb.Table(
            columns=["Group Type", "Metric", "Mean", "Std", "Abs Disparity", "Coeff Var"],
            data=summary_rows
        )
